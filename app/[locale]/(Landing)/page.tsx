  ...
  return (
    <div dir={direction} className="min-h-svh bg-background text-foreground">
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Button asChild variant="outline" size="sm" className="hidden sm:inline-flex">
              <Link href={`/${nextLocale}`}>{t.nav.switchLocaleLabel}</Link>
            </Button>
      ...
      <main id="home" className="relative overflow-hidden pt-24">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-6 pb-16 pt-12 lg:grid-cols-2 lg:pb-20 lg:pt-20">
          <div className="space-y-8 text-center lg:text-start">
            ...
            <p className="mx-auto max-w-xl text-pretty text-base leading-relaxed text-muted-foreground lg:mx-0">
              {t.hero.description}
            </p>
          ...
          <div className="relative">
            ...
            >
              ...
              <CardContent className="space-y-6">
                <div className="mx-auto flex size-48 flex-col items-center justify-center rounded-full border bg-muted/30">
                  <div className="text-3xl font-semibold">{t.metrics.status}</div>
            ...
            <div className="pointer-events-none absolute -left-4 -top-4 hidden size-12 place-items-center rounded-xl border bg-card/50 text-accent shadow-xl backdrop-blur md:grid">
              <span aria-hidden className="text-xl font-semibold">
                ₿
              </span>
            </div>
            <div className="pointer-events-none absolute -bottom-4 -right-4 hidden size-12 place-items-center rounded-xl border bg-card/50 text-primary shadow-xl backdrop-blur md:grid">
              <span aria-hidden className="text-xl font-semibold">
                ✓
              </span>
            </div>
        ...
        <section id="features" className="mx-auto max-w-7xl px-6 py-24">
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            <FeatureCard title={t.features.items.auditor.title} description={t.features.items.auditor.description} />
            <FeatureCard
              title={t.features.items.purification.title}
              description={t.features.items.purification.description}
              highlighted
            />
            <FeatureCard title={t.features.items.contracts.title} description={t.features.items.contracts.description} />
            <FeatureCard title={t.features.items.zakat.title} description={t.features.items.zakat.description} />
          </div>
        </section>
        ...
        <section id="pricing" className="mx-auto max-w-7xl px-6 pb-16">
          ...
          <div className="mx-auto mt-14 grid max-w-5xl gap-6 md:grid-cols-3 md:gap-8">
            <PricingCard
              title={t.pricing.tiers.startup.title}
              price={t.pricing.tiers.startup.price}
              period={t.pricing.period}
              features={t.pricing.tiers.startup.features}
              cta={t.pricing.tiers.startup.cta}
              variant="outline"
            />
            <PricingCard
              title={t.pricing.tiers.growth.title}
              price={t.pricing.tiers.growth.price}
              period={t.pricing.period}
              features={t.pricing.tiers.growth.features}
              cta={t.pricing.tiers.growth.cta}
              highlightLabel={t.pricing.tiers.growth.highlightLabel}
              variant="primary"
            />
            <PricingCard
...
function PricingCard({
  ...
  return (
    ...
    >
      {highlightLabel ? (
        <div className="absolute top-0 left-0 rounded-br-lg rounded-tl-xl bg-accent px-3 py-1 text-[10px] font-bold uppercase text-accent-foreground">
          {highlightLabel}
        </div>
      ) : null}

      <CardHeader className={cn(variant === "primary" && "pt-10")}>
        <CardTitle className={cn(variant === "primary" && "text-accent")}>{title}</CardTitle>
        <div className="mt-2 flex items-end gap-2">
          <div className="text-4xl font-semibold tracking-tight">{price}</div>
          {period ? <div className="pb-1 text-sm text-muted-foreground">{period}</div> : null}
        </div>
      </CardHeader>
